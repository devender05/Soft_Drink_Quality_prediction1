...
if st.button("Predict Profitability", type="primary"):
    ...
    if len(predictions) > 1:
        ...
        # Extra Graph: Scatter plot of Revenue vs Units Sold, colored by Prediction
        st.subheader("Revenue vs Units Sold (by Prediction)")
        fig, ax = plt.subplots()
        scatter = ax.scatter(result_df["Revenue"], result_df["Units Sold"],
                            c=result_df["Prediction"].map({"Profitable": "green", "Loss-Making": "red"}),
                            s=100, alpha=0.6)
